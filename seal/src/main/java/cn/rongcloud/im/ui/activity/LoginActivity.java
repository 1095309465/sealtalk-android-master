...
import cn.rongcloud.im.ui.adapter.LoginAdapter;
...
public class LoginActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void initView() {
        mList = new ArrayList<>();
        mList.add("15757855214");
        mList.add("18721887467");
        mList.add("18721064516");
        mList.add("13917488197");
        mList.add("13052202276");
        mList.add("13816328202");
        ...
        adapter = new LoginAdapter(mList, LoginActivity.this);
        lv.setAdapter(adapter);
        lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                String zhanghao = mList.get(position);
                mPhoneEdit.setText(zhanghao);
        ...
        if (result != null) {
            switch (requestCode) {
                case LOGIN:
                    ...
                    Log.e("123", "onSuccess——LOGIN=" + loginResponse.toString());
                    if (loginResponse.getCode() == 200) {
                        loginToken = loginResponse.getResult().getToken();
                        if (!TextUtils.isEmpty(loginToken)) {//连接服务器
